...
export default function AssignmentCard({ project, onSelect }) {
  ...
  return (
    <motion.article variants={blurSlideUp} whileHover={reduceMotion ? undefined : cardHover} className="assignment-premium-card assignment-cinematic-card motion-hover-depth group relative flex min-h-[460px] flex-col overflow-hidden rounded-[28px] border border-cyan-100/[0.12] shadow-[0_32px_110px_rgba(0,0,0,0.34)]">
      <div className="relative h-48 overflow-hidden border-b border-cyan-100/[0.08] bg-[#030909]">
        {project.evidenceImg && project.evidenceImg !== 'Sẽ cập nhật sau' ? (
          <motion.img src={project.evidenceImg} alt={`Minh chứng ${project.title}`} className="h-full w-full object-cover opacity-80 saturate-[0.7]" whileHover={reduceMotion ? undefined : { scale: 1.07, opacity: 1, filter: 'saturate(1)' }} transition={{ duration: 0.55, ease: [0.25, 0.46, 0.45, 0.94] }} />
        ) : (
        ...
        <div className="absolute bottom-3 left-4 flex items-center gap-2 rounded-full border border-cyan-100/15 bg-[#03100d]/75 px-2.5 py-1 font-mono text-[9px] uppercase tracking-[0.14em] text-cyan-50/80 backdrop-blur-xl"><FileText size={11} className="text-cyan-200" /> Assignment {String(project.id).padStart(2, '0')}</div>
      </div>

      <div className="relative flex flex-1 flex-col p-5 sm:p-6">
        <div className="pointer-events-none absolute -right-12 top-0 h-32 w-32 rounded-full bg-emerald-300/[0.045] blur-3xl" />
