...
export default function HomeTab({ setActiveTab }) {
  ...
  return (
    <div data-print-section className="home-luxury-shell space-y-0 pb-0">
      <Hero student={student} setActiveTab={setActiveTab} />

      <section data-print-section className="home-story-section relative overflow-hidden px-3 py-24 sm:px-5 sm:py-28 lg:px-10 lg:py-36">
        <div data-print-hidden className="absolute inset-0 bg-[radial-gradient(circle_at_16%_30%,rgba(16,185,129,0.1),transparent_28%),radial-gradient(circle_at_88%_70%,rgba(34,211,238,0.08),transparent_30%)]" />
        <div data-print-hidden className="luxury-noise absolute inset-0 opacity-[0.025]" />

        <motion.div data-print-section initial={reduceMotion ? false : 'hidden'} whileInView="visible" viewport={motionViewport} variants={blurSlideUp} className="relative mx-auto max-w-[1280px]">
          ...
          <motion.div data-print-section data-print-grid data-print-home-grid initial={reduceMotion ? false : 'hidden'} whileInView="visible" viewport={motionViewport} variants={staggerGrid} className="mt-16 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
            {overview.map((item, index) => {
              const Icon = item.icon;
              return (
                <motion.article data-print-card data-print-small-card key={item.id} variants={blurSlideUp} whileHover={reduceMotion ? undefined : cardHover} className="competency-cinematic-card group relative min-h-72 overflow-hidden rounded-[26px] border border-cyan-100/[0.09] bg-[#04100e]/92 p-6 shadow-[0_16px_44px_rgba(0,0,0,0.26)]">
                  <div data-print-hidden className="absolute -right-12 -top-12 h-40 w-40 rounded-full bg-[radial-gradient(circle,rgba(103,232,249,0.1),transparent_68%)] opacity-60 transition-opacity duration-500 group-hover:opacity-100" />
